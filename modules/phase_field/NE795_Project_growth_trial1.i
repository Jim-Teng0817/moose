...
X = 255 # the Domain/Bar length     128
...
#     type = ParsedFunction
#     prop_names = 'T'
#     function = 'T0 + (Q / (2 * pi * kappa_T)) * (1 / sqrt((x -x0 - vt)^2 + y^2)) * exp(-v * (sqrt((x -x0 - vt)^2 + y^2)+(x -x0 - vt)) / (2 * alpha))'
#     #Assume the heat source is on the top surface and move along x direction
#     args = 'x y t'
#     params = 'Q=25 kappa_T=2.75e-5 v=1e6 alpha=5.2e6 x0 = 117' 
#     # As given, ambient temperature T0 = 300 [K], location of heat source @ t=0 x0 =117 [mu-m]
#     # absorbed power from heat source Q = 25 [W], thermal conductivity kappa_T = 2.75e-5 [W*(mu-m)*K],
#     # Thermal diffusivity alpha = 5.2e6 [(mu-m)^2/s] (kappa_T/(rho*c_p)​), pulling velocity of heat source v = 1e6 [(mu-m)^2/s]
#   [../]
# []
...
[Outputs]
  file_base = /home/tsu-chun/projects_moose/moose/modules/phase_field/NE795_Project_growth_trial1/test1
  exodus = true
  csv = true
  # interval = 1
[]
