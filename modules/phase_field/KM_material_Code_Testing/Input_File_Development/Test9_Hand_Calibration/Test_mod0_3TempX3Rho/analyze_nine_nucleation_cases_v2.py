...
def build_case_summaries(
    ...
    for (case, case_dir_text), group in data.groupby(["case", "case_dir"], sort=True):
        ...
        min_dt = min_finite(group.loc[group["dt_actual"] > 0, "dt_actual"])
...
def plot_curves(
    ...
    *,
    ...
    """Plot robust comparison curves.

    Discrete counters use step plots with markers. Physical-time comparisons use
    a symmetric-log x axis so cases spanning 1e-2 to 1e2 s remain visible.
    """
    ...
    discrete_metrics = {
        ...
        "cum_deletions",
        "net_insertion_events",
        "active_nucleus_sites",
        "grain_count_early",
        "grain_count_established",
        "new_grains_early_proxy",
        "new_grains_established_proxy",
    }

    def draw_curve(curve: pd.DataFrame, metric: str, label: str) -> bool:
        clean = curve[[x_column, metric]].replace([np.inf, -np.inf], np.nan)
        clean = clean.dropna(subset=[x_column, metric]).sort_values(x_column)
        if clean.empty:
            return False
        marker_every = max(1, len(clean) // 18)
        ...
        # Mark the last available point so incomplete/short curves remain visible.
        plt.scatter(
            [clean[x_column].iloc[-1]],
            [clean[metric].iloc[-1]],
            s=24,
            zorder=5,
        )
    ...
    def finish_axis(group: pd.DataFrame, metric: str) -> None:
        if metric == "dt_actual":
            positive = pd.to_numeric(group[metric], errors="coerce")
            positive = positive[np.isfinite(positive) & (positive > 0)]
            if not positive.empty:
                plt.yscale("log")
        if x_column == "time":
            positive_x = pd.to_numeric(group[x_column], errors="coerce")
            positive_x = positive_x[np.isfinite(positive_x) & (positive_x > 0)]
            if not positive_x.empty:
                linthresh = max(float(positive_x.min()) / 2.0, 1.0e-12)
    ...
    for metric, label in metrics.items():
        ...
        for temperature, group_t in data.groupby("T"):
            ...
            for rate, curve in group_t.groupby("strain_rate"):
                plotted |= draw_curve(curve, metric, f"{rate:g} s$^{{-1}}$")
            if not plotted:
                plt.close()
                continue
            plt.xlabel(x_label)
        ...
        for rate, group_r in data.groupby("strain_rate"):
            ...
            for temperature, curve in group_r.groupby("T"):
                plotted |= draw_curve(curve, metric, f"{temperature:g} K")
            if not plotted:
                plt.close()
                continue
            plt.xlabel(x_label)
...
def plot_final_distributions(
    ...
    """Plot final-available number CDFs without hiding single-grain cases."""
    if not final_distributions:
        return
    dist_dir = output_dir / "final_grain_size_distributions"
    dist_dir.mkdir(parents=True, exist_ok=True)

    lookup = summary.set_index("case")[[
        "T",
        "strain_rate",
        "run_complete",
        "final_accumulated_strain",
    ]].to_dict("index")
    ...
    for temperature in sorted(summary["T"].dropna().unique()):
        ...
        for case, frame in final_distributions.items():
            ...
            if metadata is None or metadata["T"] != temperature or frame.empty:
                continue
            diameter = np.sort(frame["D"].dropna().to_numpy(dtype=float))
            diameter = diameter[np.isfinite(diameter) & (diameter > 0)]
            if len(diameter) == 0:
                continue

            complete = bool(metadata.get("run_complete", False))
            final_strain = float(metadata.get("final_accumulated_strain", np.nan))
            suffix = "" if complete else f" (partial, eps={final_strain:.3g})"
            label = f"{metadata['strain_rate']:g} s$^{{-1}}${suffix}"

            if len(diameter) == 1:
                # A line with one point is invisible without an explicit marker.
                plt.vlines(diameter[0], 0.0, 1.0, linewidth=1.5, linestyles="dashed")
                plt.scatter([diameter[0]], [1.0], s=42, label=label, zorder=5)
            else:
