...
# Filename
Folder_name = 'Output/'
mod_num = 1_t1_Pnuc2e-3_GBmob1e-5_rhoBasedPnuc_w25_1073K
...
nuc_radius_i = 60.0         # radius in mesh length units; start around one initial element  # 4.0
...
nuc_prob_i = 2.0e-2         # stochastic insertion probability/rate density   # 1.0e-4 5.0e-4 2.0e-3
...
# Nucleation Force
nuc_strength_i = 1000.0       # Nucleation Force, here adjust for making bnds 1 # 200
...
[Materials]
  ...
  [nucleation_probability]
    ...
    expression = 'if(0.5 * G * b * b * rho_eff * rho_scale < sigma / r_subgrain, 0, if(bnds > bnds_min, if(bnds < bnds_max, P0 * factor_n * (rho_eff / rho_ref), 0), 0))'
    outputs = exodus
  []
...
#   []
  ...
  # ---------------------------------------------------------------
  # Stored-energy versus curvature-penalty diagnostics
  # ---------------------------------------------------------------

  [stored_energy_avg]
    type = ElementAverageMaterialProperty
    mat_prop = stored_energy_density
    execute_on = 'INITIAL TIMESTEP_END'
  []

  [curvature_penalty_avg]
    type = ElementAverageMaterialProperty
    mat_prop = curvature_penalty
    execute_on = 'INITIAL TIMESTEP_END'
  []

  [energy_margin_min]
    type = ElementExtremeMaterialProperty
    mat_prop = energy_margin
    value_type = min
    execute_on = 'INITIAL TIMESTEP_END'
  []
  ...
  [energy_preferred_fraction]
    type = ElementAverageMaterialProperty
    mat_prop = energy_preferred
    execute_on = 'INITIAL TIMESTEP_END'
  []

  [eligible_nucleation_fraction]
    type = ElementAverageMaterialProperty
    mat_prop = eligible_nucleation_region
    execute_on = 'INITIAL TIMESTEP_END'
  []

  [P_nuc_unfavorable_max]
    type = ElementExtremeMaterialProperty
    mat_prop = P_nuc_unfavorable
    value_type = max
    execute_on = 'INITIAL TIMESTEP_END'
  []
...
[Outputs]
  ...
  []
  file_base = ${Folder_name}/default/result
[]
