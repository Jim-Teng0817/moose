...
# Filename
Folder_name = 'Output/'
mod_num = 30_X_withNuc_uniform_rho
...
nuc_radius_i = 20.0         # radius in mesh length units; start around one initial element  # 4.0
...
nuc_prob_i = 2.0e-4         # stochastic insertion probability/rate density   # 1.0e-4 5.0e-4 2.0e-4
...
int_width_i = 40.0            # GB interface width for new formed grains (in length scale) # 2.0 40.0
...
wGB_i = 40.0                 # GB width for initial grains (in length scale) # 4.0 40.0
...
T_i = 1073                   # Initial / Input Temperature in K
...
#   # Do not adapt every step while debugging.
#   interval = 5
...
[Outputs]
  ...
  []
  exodus = true
  file_base = ${Folder_name}/${nuc_radius_i}mum/mod${mod_num}_${input_text_file}
[]
