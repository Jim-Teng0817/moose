...
def load_data(root):
    ...
    for csv_path in files:
        if any(part in ('logs', 'comparison_plots', 'diagnostic_plots', 'diagnostic_plots_calib') for part in csv_path.parts):
            continue
        try:
...
def main():
    ...
    ap.add_argument('--root', type=Path, default=Path('CalibrationSweep_rho1e16'))
    args = ap.parse_args()
    out_dir = args.root / 'diagnostic_plots_calib'
    out_dir.mkdir(exist_ok=True, parents=True)
    data, skipped = load_data(args.root)
    summary = case_summary(data)
    data.to_csv(out_dir / 'combined_diagnostic_data_calib.csv', index=False)
    summary.to_csv(out_dir / 'corrected_case_summary_calib.csv', index=False)
    if skipped:
    ...
    print('Wrote:', out_dir)
    print('Summary:', out_dir / 'corrected_case_summary_calib.csv')
