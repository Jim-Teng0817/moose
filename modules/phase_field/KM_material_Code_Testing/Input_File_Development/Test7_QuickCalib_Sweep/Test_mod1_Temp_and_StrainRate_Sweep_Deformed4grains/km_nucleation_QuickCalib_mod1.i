...
# dt_i = 1e-3                      # 1e-5
# dt_nuc_i = 1e-3                  # 5e-6
# hold_time_i = 1e-2               # 3e-6 1e-3

# end_time_i = '${fparse 1000 * (1/time_scale_i)}'    # 10

# Strain Rate
strain_rate_i = 1e0         # Strain Rate in 1/s  1e-1 1e0 1e1 1e2 1e3
delta_strain_i     = 5e-3
delta_strain_nuc_i = 5e-3
hold_strain_i = 0.05 
total_strain_i     = 5.0           # for quick screening 5.0, more valid: 20.0
dt_i        = ${fparse delta_strain_i / strain_rate_i}
dt_nuc_i    = ${fparse delta_strain_nuc_i / strain_rate_i}
hold_time_i = ${fparse hold_strain_i / strain_rate_i}
end_time_i  = ${fparse total_strain_i / strain_rate_i}
...
# Filename
Folder_name = 'Output/'
mod_num = 7_873K_gdot_1p0_Pnuc2e-3_w10
# 6_873K_gdot_1p0_Pnuc2e-3_w10
# 6_873K_gdot_1000p0_Pnuc2e-3_w10
# 6_1073K_gdot_100p0_Pnuc2e-3_w10
# 6_1273K_gdot_10p0_Pnuc2e-3_w10
...
nuc_radius_i = 40.0         # radius in mesh length units; start around one initial element  # 4.0
...
bnds_min_i = 0.55          # Rule out the triple junction 
bnds_max_i = 0.95          # 0.95 for boundary only, 0.85 wider GB band, 0.75 moderate, 0.65 narrower, 0.55 very close to GB center
nuc_prob_i = 2.0e-3         # stochastic insertion probability/rate density   # 1.0e-4 5.0e-4 2.0e-3
...
rho_init_i = 1e16             # Added for Nucleation (Jim July 16, 2026) # in 1/m^2

# Nucleation Force
nuc_strength_i = 250.0       # Nucleation Force, here adjust for making bnds 1 # 200

# Nucleated Grain Width
int_width_i = 10.0            # GB interface width for new formed grains (in length scale) # 2.0 40.0 10.0 25.0
...
wGB_i = 10.0                 # GB width for initial grains (in length scale) # 4.0 40.0 10.0 25.0
...
T_i = 873                   # Initial / Input Temperature in K  873, 973, 1073, 1173, 1273 K
...
[GlobalParams]
  ...
  deformed_grain_num = ${fparse total_op_num_i - reserve_op_num_i}    # Modified for Nucleation (Jim June 19, 2026)
...
[Materials]
  ...
  [nucleation_probability]                # Added for Nucleation Criteria in a function of dislocation density (Jim July 8, 2026)
    type = ParsedMaterial
    property_name = P_nuc
    # coupled_variables = 'rho_grain bnds'
    # bnds is usually an AuxVariable / nonlinear variable, so keep it here
    ...
    # rho_eff is a material property produced by DeformedGrainMaterial
    material_property_names = 'rho_eff'
    constant_names = 'G b sigma r_subgrain bnds_max P0 rho_scale rho_ref factor_n'
    constant_expressions = '${G_shear_i} ${burger_i} ${GBenergy_i} ${r_subgrain_i} ${bnds_max_i} ${nuc_prob_i} ${rho_scale_i} ${rho_ref_i} ${factor_n_i}'

    # f_disloc  = 0.5 * G * b^2 * rho
    # f_subgrain = (sigma / r_subgrain) for 2D and (2 * sigma / r_subgrain) for 3D, not (sigma / (2 * r_subgrain))
    # loc = 1 if bnds < bnds_max, otherwise 0
    #
    # if f_disloc < f_subgrain:
    #   P_nuc = 0
    # else:
    #   P_nuc = P0 * loc(0 or 1) * rho
    #
    expression = 'if(0.5 * G * b * b * rho_eff * rho_scale < sigma / r_subgrain, 0, if(bnds < bnds_max, P0 * rho_eff, 0))'
  ...
  # [gbmobility_cal]
  #   type = ParsedMaterial
  #   property_name = gbmobility_cal
  #   coupled_variables = 'T'
  #   constant_names = 'GBmobility0 Q_GG_act R'
  #   constant_expressions = '${GBmob0_i} ${Q_GG_act_i} ${R_gas_const}'
  #   expression = 'GBmobility0 * exp(-Q_GG_act / (R * T))'
  #   block = 0
  #   outputs = exodus
  # []  
