...
nuc_radius_i = 4.0          # radius in mesh length units; start around one initial element
rho_crit_i = 5.0e14         # m^-2; tune after checking rho_grain output
nuc_prob_i = 1.0e-4         # stochastic insertion probability/rate density
...
bnds_crit_i = -1.0                # Added for Nucleation (Jim June 17, 2026)
...
[Mesh]
  ...
  ymin = 0.0
  ymax = 64.0
  uniform_refine = 3 # Initial uniform refinement of the mesh. # 2
[]
...
[GlobalParams]
  ...
  # Important:                    
  # Initial deformed grains are IDs 0,1,2,3.
  # New nuclei should get IDs >= 4 and should be treated as recrystallized / undeformed.
  deformed_grain_num = ${initial_op_num_i}    # Added for Nucleation (Jim June 17, 2026)
  # grain_num = ${initial_op_num_i} # Added for Nucleation (Jim June 17, 2026)
...
[UserObjects]
  [voronoi]                               # Added for DeformedGrain (Jim June 5, 2026)
    ...
    grain_num = ${initial_op_num_i}       # Added for Nucleation (Jim June 17, 2026)
    op_num = ${initial_op_num_i}          # Added for Nucleation (Jim June 17, 2026)
...
[Materials]
  ...
  [nucleation_probability]             # Added for Nucleation (Jim June 17, 2026)
    ...
    constant_names = 'rho_crit bnds_min P0'
    constant_expressions = '${rho_crit_i} ${bnds_crit_i} ${nuc_prob_i}'

    # Kocks-Mecking rho criterion:
    # Nucleate where rho_grain > rho_crit.
    # Optional grain-boundary mask: bnds > bnds_min.
    expression = 'if(rho_grain > rho_crit, if(bnds > bnds_min, P0, 0), 0)'
