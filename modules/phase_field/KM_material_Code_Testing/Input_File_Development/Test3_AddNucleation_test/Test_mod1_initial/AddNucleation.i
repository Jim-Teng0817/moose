...
[GlobalParams]
  op_num = 4            # 9  # example: 8 normal OPs + 1 reserved OP
  var_name_base = gr
  grain_tracker = grain_tracker

  # Needed if using the stored-energy action/material
  deformed_grain_num = 4      # 100
  grain_num = 4
  time_scale = ${time_scale_i}
  length_scale = ${length_scale_i}  # 1e-6     # example: mesh unit = micron; choose your actual scale
[]
...
[UserObjects]
  # Your existing polycrystal IC user object, for example:
  [voronoi]
  ...
  # Discrete nucleation site insertion
  [nuc_inserter]
    type = DiscreteNucleationInserter
    probability = P_nuc       # material property from your nucleation criterion
    radius = r_nuc            # constant or material property; use mesh length units
    hold_time = 0             # important for GrainTracker remapping
  ...
  [nuc_map]
    ...
    periodic = gr0            # use an OP with same periodicity settings
    int_width = 2.0           # smooth interface width for inserted nucleus
  []
[]
...
[Postprocessors]
  ...
  [grain_tracker]
    ...
    # Reserve one OP for new nuclei. Verify that this OP starts at zero.
    reserve_op = 1
