...
rho_crit_i = 5e-4           # nm^-2
bnds_max_i = 0.75           # 0.95 for boundary only, 0.85 wider GB band, 0.75 moderate, 0.65 narrower, 0.55 very close to GB center
nuc_prob_i = 1.0e-4         # stochastic insertion probability/rate density   # 1.0e-4 5.0e-4
...
G_shear_i = 32.7              # Added for Nucleation (Jim June 17, 2026) # 2024_Sourabh B. Kadambi  # in GPa
theta_0_i = 0.1635            # Added for Nucleation (Jim June 17, 2026) # in GPa
M_taylor_i = 2.75             # Added for Nucleation (Jim June 17, 2026) 
alpha_taylor_i = 0.3          # Added for Nucleation (Jim June 17, 2026)

# Nucleation Force
nuc_strength_i = 2.0       # Nucleation Force, here adjust for making bnds 1 # 200
...
[Materials]
  ...
  [k1]
    ...
    expression = '2 * theta_0 / (M_taylor * alpha_taylor * G * b)'       # Convert unit of b from m to nm
    block = 0
    outputs = exodus
  []      
