...
## dislocation density
Disloc_Density = '${units 9.0e15 m^-2}'

Folder_name = 'AddDeformedGrain/3_test_length_scale1e-6'           
...
[GlobalParams]
  ...
  grain_num = 36     # Number of grains (previously in voronoi) (Jim June 01, 2026)
  deformed_grain_num = 16 # From DeformedGrain (Jim June 01, 2026)
  grain_tracker = grain_tracker # From DeformedGrain (Jim June 01, 2026)
  length_scale = '${length_scale_i}' # length scale (previously in Tungsten GBEvolution) (Jim June 01, 2026)
  time_scale = '${time_scale_i}'     # time scale (previously in Tungsten GBEvolution) (Jim June 01, 2026)
  GBenergy = '${GBEnergy_scaled}'    # GB energy (previously in Tungsten GBEvolution) (Jim June 01, 2026)
  GBE = '${GBEnergy_scaled}'         # GB energy (previously in Tungsten GBEvolution) (Jim June 01, 2026)
  GBMobility = '${M_GB_i}'           # GB mobility (previously in Tungsten GBEvolution) (Jim June 01, 2026)
  wGB = '${wGB_i}'                   # GB / Diffuse Interface width (previously in Tungsten GBEvolution) (Jim June 01, 2026)
  int_width = '${wGB_i}'             # GB / Diffuse Interface width (previously in Tungsten GBEvolution) (Jim June 01, 2026)
  Disloc_Den = '${Disloc_Density}'   # From DeformedGrain (Jim June 01, 2026)
...
[UserObjects]
  ...
  #   int_width = 7
  # []
  [voronoi]                    # From DeformedGrain (Jim June 01, 2026)
    type = PolycrystalVoronoi
    # rand_seed = 81               # Mod. from Grain Growth Model (Jim June 01, 2026)
    rand_seed = 10                 # Mod. from Grain Growth Model (Jim June 01, 2026)
    # coloring_algorithm = bt    # Mod. from Grain Growth Model (Jim June 01, 2026)
    int_width = 7                # Mod. from Grain Growth Model (Jim June 01, 2026)
  []
  # [grain_tracker]
  #   type = GrainTracker
  #   compute_var_to_feature_map = true       # added for postprocessor (Jim 20250512)
  # []
  [grain_tracker]               # From DeformedGrain (Jim June 01, 2026)
    type = GrainTracker
    # threshold = 0.2
...
[Variables]                  # From DeformedGrain (Jim June 01, 2026)
...
[Kernels]                 # From DeformedGrain (Jim June 01, 2026)
  [PolycrystalKernel]
  []
...
[Materials]
  [Tungsten]
    ...
    outputs = exodus
  []
  [./deformed]             # From DeformedGrain (Jim June 01, 2026)
...
[Outputs]
  ...
  csv = true
  file_base = ${Folder_name}/mod4_${temperature}K_TimeScale${time_scale_i}_LengthScale_${length_scale_i}_wGB${wGB_i}_EnergyScale${energy_scale_i}_dt${dt_i}/grain_growth_2D_${temperature}K_TimeScale${time_scale_i}_LengthScale_${length_scale_i}_wGB${wGB_i}_EnergyScale${energy_scale_i}_dt${dt_i}
  ...
  [vector_postproc]                                      # Added to not output every time step  (Jim 20250513)
    ...
    file_base = ${Folder_name}/mod4_${temperature}K_TimeScale${time_scale_i}_LengthScale_${length_scale_i}_wGB${wGB_i}_EnergyScale${energy_scale_i}_dt${dt_i}/grain_growth_2D_${temperature}K_TimeScale${time_scale_i}_LengthScale_${length_scale_i}_wGB${wGB_i}_EnergyScale${energy_scale_i}_dt${dt_i}_grainSize
  []
