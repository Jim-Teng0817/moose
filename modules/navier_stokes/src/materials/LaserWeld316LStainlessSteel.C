...
LaserWeld316LStainlessSteel::validParams()
{
  ...
  params.addParam<Real>("c_mu0", 2385.2, "mu0 coefficient in exp(m0/T-mu1)");    // viscosity
  ...
  params.addParam<Real>("c_k0_s", 6.38, "k0 coefficient for the solid in k0+k1*T");   // thermal conductivity
  ...
  params.addParam<Real>("c_cp0_s", 459.29, "cp0 coefficient for the solid in cp0+cp1*T");   // heat capacity
