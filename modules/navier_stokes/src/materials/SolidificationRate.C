...
SolidificationRate::computeQpProperties()
{
  ...
  _cooling_rate[_qp] = - MetaPhysicL::raw_value(_temp_dot[_qp]);   // Modified for positive cooling rate during cooling (by Jim 20251024)
  ...
  // if (_liquidus_time[_qp] > 1.0e-8 && _solidus_time[_qp] > 1.0e-8)    // Modified for solidification rate calculation (by Jim 20251021)
  ...
  //   _solidus_time[_qp] < 1.0e-8)                                        // Modified for solidification rate calculation (by Jim 20251021)
  //   // _solidification_rate[_qp] = 1.0 / _temp_gradient[_qp] * std::abs( _cooling_rate[_qp] );
  //   _solidification_rate[_qp] = 1.0 / _temp_gradient[_qp] * _cooling_rate[_qp];     // Modified for +/- (- doesn't count) (by Jim 20251024)
  // else
  //   _solidification_rate[_qp] = 0.0;
  ...
  _solidification_rate[_qp] = 1.0 / _temp_gradient[_qp] * _cooling_rate[_qp];     // Modified for calculating R for all time (- doesn't count) (by Jim 20251024)
