...
INSADMassAdditionBoundaryBC::validParams()
{
  ...
  // params.addParam<Real>("cutoff_distance", 100e-6, "Cutoff Distance (m) for Mesh Displacement"); // Added for ALE Mod. PFR (Jim Nov. 19, 2025)
...
INSADMassAdditionBoundaryBC::computeQpResidual()
{
    ...
    const ADReal v_dep_top = _v_dep * (_F/_v_s);  // simple linear scaling with powder rate (linear mass density)
