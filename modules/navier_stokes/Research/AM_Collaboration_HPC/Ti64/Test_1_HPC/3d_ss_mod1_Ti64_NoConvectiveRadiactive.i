...
timestep = 10e-6 # Time step size  # 1.25e-5
...
[BCs]
  ...
  [weld_flux]  # boundary condition for laser heat flux at the front (z=max) boundary
    ...
    P0 = 100                              # Peak power of the laser in W  159.96989792079225   540 
...
[Functions]
  [path_x]
    type = ParsedFunction
    expression = "5e-3" # 2*cos(2.0*pi*t) 8.47e-3*t  6.35e-3*t  10.58e-3*t  "8.47e-3*t + 0.01"  "8.47e-3*t + 2.1175e-3"
  []
  [path_y]
    type = ParsedFunction
    expression = "5e-3" # 2*sin(2.0*pi*t)   0  0.0012  
  []
  [path_z]
    type = ParsedFunction
