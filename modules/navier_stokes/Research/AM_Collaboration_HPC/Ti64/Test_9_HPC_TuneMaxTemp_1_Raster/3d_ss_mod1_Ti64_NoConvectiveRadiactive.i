...
period = 0.5 # Period of the laser motion  # 1.25e-3  5.00e-1 10.6e-1  Dwell time = 160 mus, simulation time = 320 mus   800e-6
...
Effective_EBeam_Power = 25                # EBeam Power * EBeam Absorption Efficiency      # 1680 * 80% = 1344   5kV * 28mA * 80% = 112  25
EBeam_Scan_Speed = 4.550e-3  # EBeam Scan Speed in m/s  # 8.47e-3 6.35e-3 10.58e-3 
Start_Point_X = 6.5e-3  # Starting X coordinate of the laser beam in m
...
[Mesh]
  [gen] 
    ...
    ymin = 0e-3
    ymax = 15.0e-3 # 0.5e-3   1.5e-3  9.0e-3
    zmin = 0e-3
    zmax = 10.0e-3 # 0.5e-3   1.5e-3  4.5e-3 
    nx = 15  # 2  10 40
    ny = 15 # 6  10 8
    nz = 10  # 2  5 4
  []
...
[BCs]
  ...
  [weld_flux_on]
    ...
    variable = T
    boundary = 'front'
    P0 = ${Effective_EBeam_Power}                  # 1680 * 80% = 1344
    R = 1.00e-4
    x_beam_coord = path_x
    y_beam_coord = path_y
    z_beam_coord = path_z
...
[Functions]
  ...
  [path_x]
    type = ParsedFunction
    expression = "${EBeam_Scan_Speed}*t + ${Start_Point_X}" # 2*cos(2.0*pi*t) 8.47e-3*t  6.35e-3*t  10.58e-3*t  "8.47e-3*t + 0.01"  "8.47e-3*t + 2.1175e-3"
  []
  [path_y]
    type = ParsedFunction
    expression = "7.5e-3" # 2*sin(2.0*pi*t)   0  0.0012  
  []
  [path_z]
    type = ParsedFunction
    expression = "10e-3" # 1 0.001 0.0012 0.0008 0.00115 0.00125  0.01025
  []
[]
