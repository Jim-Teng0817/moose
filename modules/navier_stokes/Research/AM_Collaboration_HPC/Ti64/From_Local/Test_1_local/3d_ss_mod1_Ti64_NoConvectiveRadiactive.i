...
period =320e-6 # Period of the laser motion  # 1.25e-3  5.00e-1 10.6e-1  Dwell time = 160 mus, simulation time = 320 mus
endtime = ${period} # Total simulation time (Simulation end time)
timestep = 2e-6 # Time step size  # 1.25e-5
...
[Mesh]
  [gen] 
    ...
    dim = 3
    xmin = 0e-3
    xmax = 10.0e-3 # 1.5e-3  5.0e-3  10.0e-3
    ymin = 0e-3
    ymax = 10.0e-3 # 0.5e-3   1.5e-3  9.0e-3
    zmin = 0e-3
...
[BCs]
  ...
  [weld_flux]  # boundary condition for laser heat flux at the front (z=max) boundary
    ...
    P0 = 540                              # Peak power of the laser in W  159.96989792079225
    R = 1.00e-4                           # The radius at which the beam intensity falls to 1/e^2 of its axis value in m  1.8257418583505537e-4   1.00e-3 => 5e-4    200um = 1e-4
    x_beam_coord = '5.00e-3'             # x coordinate of the laser beam center (moving in x direction over time) Scan Speed = 4.5e-3/${endtime} '10.0e-3*t/${endtime} + 1.0e-3' 
    y_beam_coord = '5.00e-3'              # y coordinate of the laser beam center (fixed in y direction)  '0.25e-3'  '0.75e-3'
...
#   #   type = ParsedFunction
#   #   vars = 'endtime'
#   #   value = 'if(t <= endtime/2, 1, 0)'
#   # [../]
#   [./time_switch]
#     type = PiecewiseConstant
#     x = '0 160e-6 320e-6' # Switch at 160e-6 and 320e-6 seconds
#     y = 'true true false' # On between 0 and 10, then off after 10
#   [../]
# []
...
#   #   times = times_on
#   #   enable_objects = 'BCs::weld_flux'
#   #   disable_objects = 'Postprocessors::count_off'
#   #   execute_on = 'INITIAL TIMESTEP_END'
#   # [../]

#   # [./pulse_off]
#   #   type = EnableDisableMooseObjects
#   #   disable_list = 'heat_flux_control'
#   #   triggering_event = 'timestep_after_time'
#   #   timestep_after_time = 160e-6
#   # [../]
#   [./disable_flux]
#     type = TimePeriod
#     # enable_objects = 'Kernel::diff0'
#     disable_objects = 'BCs::weld_flux'
#     start_time = '160e-6'
#     end_time = '${endtime}'
...
[Materials]
  ...
  [density]
    ...
    prop_names = 'density'
    prop_values = 4430 # kg/m^3  Ti-6Al-4V density 4430 # Stainless steel density 7609 kg/m^3
  []
  [heat]
    type = ADHeatConductionMaterial
    specific_heat_temperature_function = 560 #  J/kg·K specific heat capacity of Ti-6Al-4V 560 J/kg·K # specific heat capacity of Stainless steel 500 J/kg·K
    thermal_conductivity_temperature_function = 6.7 # W/m·K thermal conductivity of Ti-6Al-4V 6.7 to 7.5 W/m·K # thermal conductivity of Stainless steel 25 W/m·K
    temp = T
  []
