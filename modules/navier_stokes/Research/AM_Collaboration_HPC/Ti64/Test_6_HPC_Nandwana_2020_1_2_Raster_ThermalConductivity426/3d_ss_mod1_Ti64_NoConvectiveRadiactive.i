...
period = 0.47 # Period of the laser motion  # 1.25e-3  5.00e-1 10.6e-1  Dwell time = 160 mus, simulation time = 320 mus   800e-6
endtime = ${period} # Total simulation time (Simulation end time)
timestep = 0.005 # Time step size  # 1.25e-5 10e-6
...
[Mesh]
  [gen] 
    ...
    dim = 3
    xmin = 0e-3
    xmax = 4.0e-3 # 1.5e-3  5.0e-3  10.0e-3
    ymin = 0e-3
    ymax = 8.0e-4 # 0.5e-3   1.5e-3  9.0e-3
    zmin = 0e-3
    zmax = 4.0e-4 # 0.5e-3   1.5e-3  4.5e-3 
    nx = 40  # 2  10
    ny = 8 # 6  10 
    nz = 4  # 2  5
  []
...
[Materials]
  ...
  [heat]
    ...
    thermal_conductivity_temperature_function = 426 # W/m·K thermal conductivity of Ti-6Al-4V 6.7 to 7.5 W/m·K (Room Temp.)   15~20 W/m·K (773 K)  27 W/m·K (for solid from Nandwana_2020) 426 W/m·K (for liquid from Nandwana_2020)  # thermal conductivity of Stainless steel 25 W/m·K
    temp = T
  []
...
[Functions]
  ...
  [path_x]
    type = ParsedFunction
    expression = "8.47e-3*t" # 2*cos(2.0*pi*t) 8.47e-3*t  6.35e-3*t  10.58e-3*t  "8.47e-3*t + 0.01"  "8.47e-3*t + 2.1175e-3"
  []
  [path_y]
    type = ParsedFunction
    expression = "4e-4" # 2*sin(2.0*pi*t)   0  0.0012  
  []
  [path_z]
    type = ParsedFunction
    expression = "4e-4" # 1 0.001 0.0012 0.0008 0.00115 0.00125  0.01025
  []
[]
