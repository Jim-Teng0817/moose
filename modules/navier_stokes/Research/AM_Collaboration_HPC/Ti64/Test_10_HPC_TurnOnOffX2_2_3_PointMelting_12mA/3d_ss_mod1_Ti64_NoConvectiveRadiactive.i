...
period = 800e-6 # Period of the laser motion  # 1.25e-3  5.00e-1 10.6e-1  Dwell time = 160 mus, simulation time = 320 mus   800e-6
...
Effective_EBeam_Power = 48.0         # EBeam Power * EBeam Absorption Efficiency      #  720 * 80% = 576    5kV * 12mA * 80% = 48
# EBeam_Scan_Speed = 10.58e-3  # EBeam Scan Speed in m/s  # 8.47e-3 6.35e-3 10.58e-3 
...
[Mesh]
  [gen] 
    ...
    dim = 3
    xmin = 0e-3
    xmax = 8.0e-3 # 1.5e-3  5.0e-3  10.0e-3
    ymin = 0e-3
    ymax = 8.0e-3 # 0.5e-3   1.5e-3  9.0e-3
    zmin = 0e-3
...
[BCs]
  ...
  [weld_flux_on]
    ...
    variable = T
    boundary = 'front'
    P0 = ${Effective_EBeam_Power}                #  720 * 80% = 576
    R = 1.00e-4
    x_beam_coord = path_x
    y_beam_coord = path_y
    z_beam_coord = path_z
  ...
  [weld_flux_on_2]
    ...
    variable = T
    boundary = 'front'
    P0 = ${Effective_EBeam_Power}                 #  660 * 80% = 528
    R = 1.00e-4
    x_beam_coord = path_x_2
    y_beam_coord = path_y_2
    z_beam_coord = path_z_2
    enable = false
  []
  [weld_flux_off_2]
    type = GaussianEnergyFluxBC
...
[Functions]
  ...
  [path_x]
    type = ParsedFunction
    expression = "4e-3" # 2*cos(2.0*pi*t) 8.47e-3*t  6.35e-3*t  10.58e-3*t  "8.47e-3*t + 0.01"  "8.47e-3*t + 2.1175e-3"
  []
  [path_y]
    type = ParsedFunction
    expression = "4e-3" # 2*sin(2.0*pi*t)   0  0.0012  
  []
  [path_z]
    type = ParsedFunction
    expression = "5e-3" # 1 0.001 0.0012 0.0008 0.00115 0.00125  0.01025
  []

  [path_x_2]
    type = ParsedFunction
    expression = "2e-3" # 2*cos(2.0*pi*t) 8.47e-3*t  6.35e-3*t  10.58e-3*t  "8.47e-3*t + 0.01"  "8.47e-3*t + 2.1175e-3"
  []
  [path_y_2]
    type = ParsedFunction
    expression = "2e-3" # 2*sin(2.0*pi*t)   0  0.0012  
  []
  [path_z_2]
    type = ParsedFunction
    expression = "5e-3" # 1 0.001 0.0012 0.0008 0.00115 0.00125  0.01025
...
[Controls]
  [flux_control_1_on] # First flux point ON (0s to 160e-6s)
    type = TimePeriod
    start_time = 0.0
    end_time = 0.160e-3 # 160e-6 s
    enable_objects = 'BCs/weld_flux_on'
    disable_objects = 'BCs/weld_flux_off BCs/weld_flux_on_2 BCs/weld_flux_off_2'
  []
  [flux_control_1_off] # First flux point OFF (160e-6s to 300e-6s)
    type = TimePeriod
    start_time = 0.160e-3 # After the first one finishes
    end_time = 0.3e-3   # Before the second one starts
    enable_objects = 'BCs/weld_flux_off'
    disable_objects = 'BCs/weld_flux_on BCs/weld_flux_on_2 BCs/weld_flux_off_2'
  []
  [flux_control_2_on] # Second flux point ON (300e-6s to 460e-6s)
    type = TimePeriod
    start_time = 0.3e-3
    end_time = 0.460e-3 # 300e-6s + 160e-6s dwell time
    enable_objects = 'BCs/weld_flux_on_2'
    disable_objects = 'BCs/weld_flux_on BCs/weld_flux_off BCs/weld_flux_off_2'
  []
  [flux_control_2_off] # Second flux point OFF (460e-6s to end)
    type = TimePeriod
    start_time = 0.460e-3 # After the second one finishes
    end_time = ${endtime}
    enable_objects = 'BCs/weld_flux_off_2'
    disable_objects = 'BCs/weld_flux_on_2 BCs/weld_flux_on BCs/weld_flux_off'
  []
[]
