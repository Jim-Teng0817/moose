...
[Controls]
  [flux_control_1_on] # First flux point ON (0 to 160e-6 s)
    type = TimePeriod
    start_time = 1e-8
    end_time   = 0.160e-3
    enable_objects  = 'BCs/weld_flux_on'
    disable_objects = 'BCs/weld_flux_on_2'
  []
  [flux_control_1_off] # Turn first OFF (160e-6 to 300e-6)
    type = TimePeriod
    start_time = 0.160e-3
    end_time   = 0.3e-3
    # enable_objects  = ''                 # nothing to turn on during this idle period
    disable_objects = 'BCs/weld_flux_on'     # ensure first is off
  []
  [flux_control_2_on] # Second flux point ON (300e-6 to 460e-6)
    type = TimePeriod
    start_time = 0.3e-3
    end_time   = 0.460e-3
  ...
  []
  [flux_control_2_off] # Second OFF (460e-6 to end)
    type = TimePeriod
    start_time = 0.460e-3
    end_time   = ${endtime}
    # enable_objects  = ''
