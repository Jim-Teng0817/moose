period = 10.6e-1 # Period of the laser motion  # 1.25e-3  5.00e-1
...
[Mesh]
  [gen] 
    ...
    dim = 3
    xmin = 0e-3
    xmax = 27.0e-3 # 1.5e-3  5.0e-3 10.0e-3
    ymin = 0e-3
    ymax = 9.0e-3 # 0.5e-3  1.5e-3  9.0e-3
    zmin = 0e-3
    zmax = 4.5e-3 # 0.5e-3
    nx = 48  # 2
    ny = 12 # 6
...
[BCs]
  ...
  [weld_flux]  # boundary condition for laser heat flux at the front (z=max) boundary
    ...
    x_beam_coord = '9.0e-3*t/${endtime} + 9.0e-3'  # x coordinate of the laser beam center (moving in x direction over time) Scan Speed = 4.5e-3/${endtime} '10.0e-3*t/${endtime} + 1.0e-3' 
    y_beam_coord = '4.50e-3'              # y coordinate of the laser beam center (fixed in y direction)  '0.25e-3'  '0.75e-3'
    z_beam_coord = '4.50e-3'              # z coordinate of the laser beam center (fixed in z direction)  '0.5e-3' 
