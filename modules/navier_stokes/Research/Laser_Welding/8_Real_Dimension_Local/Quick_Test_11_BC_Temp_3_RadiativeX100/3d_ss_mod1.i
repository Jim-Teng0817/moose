period = 2.13e0 # Period of the laser motion  # 1.25e-3  5.00e-1 10.6e-1
...
[Mesh]
  [gen] 
    ...
    dim = 3
    xmin = 0e-3
    xmax = 54.0e-3 # 1.5e-3  5.0e-3  10.0e-3
    ymin = 0e-3
    ymax = 27.0e-3 # 0.5e-3   1.5e-3  9.0e-3
    zmin = 0e-3
    zmax = 13.5e-3 # 0.5e-3   1.5e-3  4.5e-3 
    nx = 24  # 2
    ny = 12 # 6
    nz = 6  # 2
  []
...
[BCs]
  ...
  [radiation_top_1] # currently in use
    ...
    emissivity_function = '20.0' # '3/(5.670367e-8*4*300*300*300)'    0.20  2.0    0.8  # stefan boltzmann constant = 5.670367e-8 W/m^2K^4
  ...
  [weld_flux]  # boundary condition for laser heat flux at the front (z=max) boundary
    ...
    P0 = 300                              # Peak power of the laser in W  159.96989792079225
    ...
    x_beam_coord = '18e-3*t/${endtime} + 18e-3'  # x coordinate of the laser beam center (moving in x direction over time) Scan Speed = 4.5e-3/${endtime} '10.0e-3*t/${endtime} + 1.0e-3' 
    y_beam_coord = '13.50e-3'              # y coordinate of the laser beam center (fixed in y direction)  '0.25e-3'  '0.75e-3'
    z_beam_coord = '13.50e-3'              # z coordinate of the laser beam center (fixed in z direction)  '0.5e-3' 
  ...
  [displace_z_top]
    ...
    activation_temperature = 3000.00   # 2500 1708.00
