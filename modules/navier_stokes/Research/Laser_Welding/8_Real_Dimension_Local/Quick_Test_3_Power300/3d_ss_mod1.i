period = 5e-1 # Period of the laser motion  # 1.25e-3   
endtime = ${period} # Total simulation time (Simulation end time)
timestep = 1e-3 # Time step size  # 1.25e-5
...
[BCs]
  ...
  [weld_flux]  # boundary condition for laser heat flux at the front (z=max) boundary
    ...
    R = 1.8257418583505537e-4             # The radius at which the beam intensity falls to 1/e^2 of its axis value in m  1.8257418583505537e-4
    x_beam_coord = '5.0e-3*t/${endtime} + 0.5e-3'  # x coordinate of the laser beam center (moving in x direction over time) Scan Speed = 4.5e-3/${endtime}
    y_beam_coord = '0.75e-3'              # y coordinate of the laser beam center (fixed in y direction)  '0.25e-3'
    z_beam_coord = '1.5e-3'               # z coordinate of the laser beam center (fixed in z direction)  '0.5e-3' 
