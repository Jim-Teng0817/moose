...
[BCs]
  ...
  [weld_flux]  # boundary condition for laser heat flux at the front (z=max) boundary
    ...
    x_beam_coord = '0.25e-3'              # x coordinate of the laser beam center (fixed in x direction)
    y_beam_coord = '1.5e-3*t/${endtime}'  # y coordinate of the laser beam center (moving in y direction over time) 
  ...
  [displace_z_top]
    ...
    variable = 'disp_z'
    temperature = 'T'
    deposition_velocity = 150e-3          # velocity at which material is deposited, in m/s
    activation_temperature = 2500         # temperature at which deposition starts, in K (Could be material melting temperature)
    smooth_param = 200                    # numerical value for the activation function
  []
