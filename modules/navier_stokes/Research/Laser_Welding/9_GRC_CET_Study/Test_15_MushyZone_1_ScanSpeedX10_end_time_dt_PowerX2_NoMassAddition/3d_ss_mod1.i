period = 0.05e0 # Period of the laser motion  # 1.25e-3  5.00e-1 10.6e-1 0.5e0 0.05e0
endtime = ${period} # Total simulation time (Simulation end time)
timestep = 1e-3 # Time step size  # 1.25e-5 1e-2 1e-3
...
[Mesh]
  [gen] 
    ...
    dim = 3
    xmin = 0e-3
    xmax = 8.47e-3 # 1.5e-3  5.0e-3  10.0e-3 15.0e-3
    ymin = 0e-3
    ymax = 4.0e-3 # 0.5e-3   1.5e-3  9.0e-3  4.0e-3
    zmin = 0e-3
    zmax = 2.0e-3 # 0.5e-3   1.5e-3  4.5e-3  5.0e-3
    nx = 43  # 2  18  18 36 75 43
    ny = 20  # 6  12  8  16 20 20
    nz = 10  # 2   6  6  12 25 10
  []
...
[BCs]
  ...
  [weld_flux]  # boundary condition for laser heat flux at the front (z=max) boundary
    ...
    P0 = 200                               # Peak power of the laser in W  159.96989792079225  120 100 200
    ...
    x_beam_coord = '4.235e-3*t/${endtime} + 2.1175e-3'  # x coordinate of the laser beam center (moving in x direction over time) Scan Speed = 4.5e-3/${endtime} '10.0e-3*t/${endtime} + 1.0e-3'  4.235e-3
    y_beam_coord = '2.00e-3'              # y coordinate of the laser beam center (fixed in y direction)  '0.25e-3'  '0.75e-3'
    z_beam_coord = '2.00e-3'              # z coordinate of the laser beam center (fixed in z direction)  '0.5e-3' 
...
[Materials]
  ...
  [steel]
    ...
    # Tl = 1800                                     # Just to Increase Range and see trend
    # Ts = 1200                                     # Just to Increase Range and see trend
  ...
  [solidification_rate]
    ...
    solidus_temperature = 1675                    # Just to Increase Range and see trend  # 1675
    liquidus_temperature = 1708                   # Just to Increase Range and see trend  # 1708
    temperature = T
    outputs = exodus
  []
[]
