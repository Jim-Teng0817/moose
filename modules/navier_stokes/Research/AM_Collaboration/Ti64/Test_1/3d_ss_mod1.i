...
[Mesh]
  [gen] 
    ...
    dim = 3
    xmin = 0e-3
    xmax = 40.0e-3 # 1.5e-3  5.0e-3  10.0e-3
    ymin = 0e-3
    ymax = 40.0e-3 # 0.5e-3   1.5e-3  9.0e-3
    zmin = 0e-3
    zmax = 20.0e-3 # 0.5e-3   1.5e-3  4.5e-3 
    nx = 20  # 2
    ny = 20 # 6
    nz = 10  # 2
  []
...
[BCs]
  ...
  [weld_flux]  # boundary condition for laser heat flux at the front (z=max) boundary
    ...
    R = 10.00e-4                           # The radius at which the beam intensity falls to 1/e^2 of its axis value in m  1.8257418583505537e-4   1.00e-3 => 5e-4    200um = 1e-4
    x_beam_coord = '10.00e-3'             # x coordinate of the laser beam center (moving in x direction over time) Scan Speed = 4.5e-3/${endtime} '10.0e-3*t/${endtime} + 1.0e-3' 
    y_beam_coord = '10.00e-3'              # y coordinate of the laser beam center (fixed in y direction)  '0.25e-3'  '0.75e-3'
    z_beam_coord = '10.00e-3'              # z coordinate of the laser beam center (fixed in z direction)  '0.5e-3' 
